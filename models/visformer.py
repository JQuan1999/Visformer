...
class Attention(nn.Cell):
    ...
    def __init__(
        ...
        if use_bias:
            self.relative_bias = RelativeBias(window_size, num_heads)
...
class Block(nn.Cell):
    ...
    def __init__(
        ...
        if not attn_disabled:
            ...
            self.attn = Attention(dim, window_size=to_2tuple(window_size), num_heads=num_heads, head_dim_ratio=head_dim_ratio,
                                  qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, use_bias=use_bias)
...
class Visformer(nn.Cell):
    ...
    Args:
        ...
        use_bias : if true will use RelativeBias else not. Default: False.
    """
    ...
    def __init__(
        ...
        if depth[0]:
            ...
            self.stage0 = nn.CellList([
                Block(dim=embed_dim // 4, input_resolution=(img_size, img_size), num_heads=num_heads[0], window_size=56, head_dim_ratio=0.25, mlp_ratio=mlp_ratio,
                      qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i],
                      group=group, attn_disabled=(attn_stage[0] == "0"), spatial_conv=(spatial_conv[0] == "1"), use_bias=use_bias)
        ...
        self.stage1 = nn.CellList([
            Block(
                dim=embed_dim // 2, input_resolution=(img_size, img_size), num_heads=num_heads[1], window_size=28, head_dim_ratio=0.5, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i],
                group=group, attn_disabled=(attn_stage[1] == "0"), spatial_conv=(spatial_conv[1] == "1"), use_bias=use_bias
            )
        ...
        self.stage2 = nn.CellList([
            Block(
                dim=embed_dim, input_resolution=(img_size, img_size), num_heads=num_heads[2], window_size=14, head_dim_ratio=1.0, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i],
                group=group, attn_disabled=(attn_stage[2] == "0"), spatial_conv=(spatial_conv[2] == "1"), use_bias=use_bias
            )
        ...
        self.stage3 = nn.CellList([
            Block(
                dim=embed_dim * 2, input_resolution=(img_size, img_size), num_heads=num_heads[3], window_size=7, head_dim_ratio=1.0, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i],
                group=group, attn_disabled=(attn_stage[3] == "0"), spatial_conv=(spatial_conv[3] == "1"), use_bias=use_bias
            )
...
def visformer_tiny(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = Visformer(img_size=224, init_channels=16, num_classes=num_classes, embed_dim=192,
                      ...
                      attn_stage="0011", spatial_conv="1100", drop_path_rate=0.03, conv_init=True, use_bias=False,
                      **kwargs)
    if pretrained:
...
def visformer_tiny_v2(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = Visformer(img_size=224, init_channels=24, num_classes=num_classes, embed_dim=192,
                      ...
                      attn_stage="0011", spatial_conv="1100", drop_path_rate=0.03, conv_init=True, use_bias=True,
                      **kwargs)
    if pretrained:
        load_pretrained(model, default_cfg, num_classes=num_classes, in_channels=in_channels)
    return model
...
def visformer_small(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = Visformer(img_size=224, init_channels=32, num_classes=num_classes, embed_dim=384,
                      ...
                      attn_stage="0011", spatial_conv="1100", conv_init=True, use_bias=False, **kwargs)
    if pretrained:
        load_pretrained(model, default_cfg, num_classes=num_classes, in_channels=in_channels)
    return model
...
def visformer_small_v2(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = Visformer(img_size=224, init_channels=32, num_classes=num_classes, embed_dim=256,
                      ...
                      group=8, attn_stage="0011", spatial_conv="1100", conv_init=True, use_bias=True, **kwargs)
    if pretrained:
        load_pretrained(model, default_cfg, num_classes=num_classes, in_channels=in_channels)
    return model
