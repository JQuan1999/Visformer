...
def visformer_tiny(pretrained: bool = False,
    ...
    model = Visformer(img_size=224, init_channels=16, embed_dim=192, depth=[0, 7, 4, 4],
                      num_heads=[3, 3, 3, 3], mlp_ratio=4., group=8, attn_stage='0011',
                      spatial_conv='1100', drop_path_rate=0.03, conv_init=True, **kwargs)
    if pretrained:
...
def visformer_small(pretrained: bool = False,
    ...
    model = Visformer(img_size=224, init_channels=32, embed_dim=384, depth=[0, 7, 4, 4],
                      num_heads=[6, 6, 6, 6], mlp_ratio=4., group=8, attn_stage='0011',
                      spatial_conv='1100', conv_init=True, **kwargs)
    if pretrained:
        load_pretrained(model, default_cfg, num_classes=num_classes, in_channels=in_channels)
    return model
...
def visformer_small_v2(pretrained: bool = False,
    ...
    model = Visformer(img_size=224, init_channels=32, embed_dim=256, depth=[1, 10, 14, 3],
                      num_heads=[2, 4, 8, 16], mlp_ratio=4., qk_scale=-0.5, group=8,
...
def visformer_tiny_v2(pretrained: bool = False,
    ...
    model = Visformer(img_size=224, init_channels=24, embed_dim=192, depth=[1, 4, 6, 3],
                      num_heads=[1, 3, 6, 12], mlp_ratio=4., qk_scale=-0.5, group=8, attn_stage='0011',
                      spatial_conv='1100', drop_path_rate=0.03, conv_init=True, **kwargs)
    if pretrained:
        load_pretrained(model, default_cfg, num_classes=num_classes, in_channels=in_channels)
    return model
